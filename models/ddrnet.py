...
ddrnet_settings = {
    '23slim': [[32, 64, 128, 256, 512], 128, 64],
    '23': [[64, 128, 256, 512, 1024], 128, 128]
}
...
class DDRNet(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x += self.down4(F.relu(x_))
        x_ += F.interpolate(self.compression4(F.relu(layers[3])), size=(H, W), mode='bilinear', align_corners=True)
